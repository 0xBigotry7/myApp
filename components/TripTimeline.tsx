...
export default function TripTimeline({ expenses, posts, users }: TripTimelineProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-zinc-100 -mx-6 px-6 py-4 space-y-4">
        <div className="relative">
          <input
            ...
            className="w-full px-4 py-2.5 pl-10 rounded-xl bg-zinc-100 border-transparent focus:bg-white focus:border-zinc-300 focus:ring-0 transition-all text-sm"
          />
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between overflow-x-auto pb-2 sm:pb-0 scrollbar-hide">
          ...
          <div className="flex gap-2 pl-4 border-l border-zinc-100">
            <button
      ...
      {timelineItems.length === 0 && (
        <div className="text-center py-20">
          <div className="w-16 h-16 bg-zinc-100 rounded-full flex items-center justify-center mx-auto mb-4">
            {filterType === "photos" ? <Camera className="w-8 h-8 text-zinc-400" /> : 
             filterType === "expenses" ? <Receipt className="w-8 h-8 text-zinc-400" /> : 
      ...
      {/* Timeline Items */}
      {timelineItems.length > 0 && (
        <div className="relative border-l-2 border-zinc-100 ml-4 sm:ml-8 space-y-12 pb-8">
          {Object.entries(groupedByDate).map(([dateKey, items]) => (
            <div key={dateKey} className="relative pl-8 sm:pl-12">
              {/* Date Marker */}
              <div className="absolute -left-[9px] sm:-left-[9px] top-0">
                <div className="w-4 h-4 rounded-full bg-zinc-200 border-4 border-white ring-1 ring-zinc-100" />
              </div>
              ...
              <div className="mb-6 flex items-center gap-3">
                ...
                <span className="text-xs font-medium text-zinc-400 bg-zinc-50 px-2 py-1 rounded-full border border-zinc-100">
                  {items.length} items
                </span>
              </div>
              ...
              <div className="space-y-6">
                {items.map((item) => {
                  if (item.type === "expense") {
                    ...
                    return (
                      <div key={expense.id} className="group relative bg-white rounded-xl border border-zinc-100 p-4 hover:shadow-md hover:border-zinc-200 transition-all">
                        <div className="flex gap-4">
                          <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                                  <button 
                                    ...
                                    className="p-1.5 text-zinc-400 hover:text-zinc-900 rounded-md hover:bg-zinc-50"
                                  >
                                    <Edit2 className="w-3.5 h-3.5" />
                                  </button>
                                  <button 
                                    onClick={() => handleDelete("expense", expense.id)}
                                    className="p-1.5 text-zinc-400 hover:text-red-600 rounded-md hover:bg-red-50"
                                  >
                            ...
                            {isEditing ? (
                              <div className="mt-4 p-4 bg-zinc-50 rounded-lg space-y-4 border border-zinc-200">
                                {/* Edit Form (simplified for brevity, matching the structure of read file but with cleaner UI) */}
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <label className="text-xs font-medium text-zinc-500 block mb-1">Date</label>
                                    <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)} className="w-full px-2 py-1.5 rounded border border-zinc-200 text-sm" />
                                  </div>
                                  <div>
                                    <label className="text-xs font-medium text-zinc-500 block mb-1">Time</label>
                                    <input type="time" value={editTime} onChange={e => setEditTime(e.target.value)} className="w-full px-2 py-1.5 rounded border border-zinc-200 text-sm" />
                                  </div>
                                </div>
                                <input 
                                  type="text" 
                                  ...
                                  placeholder="Note"
                                  className="w-full px-2 py-1.5 rounded border border-zinc-200 text-sm"
                                />
                                <div className="flex justify-end gap-2">
                                  <button onClick={cancelEdit} className="px-3 py-1.5 text-xs font-medium text-zinc-600 hover:text-zinc-900">Cancel</button>
                                  <button onClick={handleEdit} className="px-3 py-1.5 text-xs font-medium bg-zinc-900 text-white rounded-md hover:bg-zinc-800">Save</button>
                                </div>
                              </div>
                            ) : (
                              <div className="mt-2 space-y-2">
                                {expense.note && (
                                  <p className="text-sm text-zinc-600">{expense.note}</p>
                                )}
                                
                                {(expense.location || expense.transportationMethod) && (
                                  <div className="flex flex-wrap gap-2">
                                    {expense.location && (
                                      <span className="inline-flex items-center gap-1 text-xs text-zinc-500 bg-zinc-50 px-2 py-1 rounded border border-zinc-100">
                    ...
                    return (
                      <div key={post.id} className="group relative bg-white rounded-xl border border-zinc-100 p-4 hover:shadow-md hover:border-zinc-200 transition-all">
                        <div className="flex gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <div 
                                  className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ring-2 ring-white shadow-sm"
                                  style={{ backgroundColor: badge.color }}
                                >
                                  {badge.initial}
                                </div>
                                <div>
                                  <span className="text-sm font-semibold text-zinc-900 block leading-none mb-1">{badge.name}</span>
                                  <span className="text-xs text-zinc-400 flex items-center gap-1">
                                    <Clock className="w-3 h-3" />
                                    {format(item.date, "h:mm a")}
                                    {post.location && (
                            ...
                            {isEditing ? (
                              <div className="p-4 bg-zinc-50 rounded-lg space-y-4 border border-zinc-200">
                                <div className="grid grid-cols-2 gap-3">
                                  <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)} className="w-full px-2 py-1.5 rounded border border-zinc-200 text-sm" />
                                  <input type="time" value={editTime} onChange={e => setEditTime(e.target.value)} className="w-full px-2 py-1.5 rounded border border-zinc-200 text-sm" />
                                </div>
                                ...
                                {post.photos.length > 0 && (
                                  <div className={`grid gap-2 ${post.photos.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                                    {post.photos.map((photoUrl, index) => {
                                       ...
                                       return (
                                        <div key={index} className="relative aspect-[4/3] rounded-lg overflow-hidden bg-zinc-100">
                                          <img 
                                            src={displayUrl} 
                                            alt="" 
